...
//   //尝试锚点滚动
//   document.querySelector('#five').scrollIntoView({behavior:"smooth",inline:"nearest"})
  //控制弹幕速度 随机数组
  ...
  for (let i = 0; i < 20; i++) {
    ...
    document.getElementsByClassName("member-middle-right-toLeft")[0].children[
      i
    ].style.animationDuration = speedArr[i] + "s";

    document.getElementsByClassName("member-middle-right-toRight")[0].children[
      i
    ].style.animationDuration = speedArr[i] + "s";
  }
  ...
  let falg = true;
  //鼠标在轮播图上取消默认行为
  document.getElementsByClassName("lunbokuaianzhuo")[0].onmousemove =
    function () {
      console.log("不OK");
