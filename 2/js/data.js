...
sendAjax("http://music.eleuu.com/personalized?limit=12").then(
  (value) => {
    for (let i = 0; i < 12; i++) {
      console.log(value);
...
for (let i = 0; i < 12; i++) {
  a[i].onclick = () => {
    sendAjax("http://music.eleuu.com/playlist/detail?id=" + idCode[i]).then(
      (value) => {
        for (let j = 0; j < 10; j++) {
          songName[j + 1].innerHTML = value.playlist.tracks[j].name;

          ablumn[j + 1].innerHTML =
            "《" + value.playlist.tracks[j].al.name + "》";
          if (value.playlist.tracks[j].ar.length > 1)
            singer[j + 1].innerHTML =
              value.playlist.tracks[j].ar[0].name +
              "<br>" +
              value.playlist.tracks[j].ar[1].name;
          else {
          ...
          songName[j + 1].onclick = function () {
            sendAjax("http://music.eleuu.com/lyric?id=" + idCodeWord[j]).then(
              (value) => {
